...
.. list-table::
   ...
   * - `Client ID Metadata Document
       ...
       (CIMD, IETF draft)
     - Opt-in
     - ``CIMD_ENABLED`` lets a client use an HTTPS URL as its ``client_id`` (an alternative
       to RFC 7591 DCR, preferred by the MCP profile); advertised as
       ``client_id_metadata_document_supported`` in the server metadata.
   ...
   * - `RFC 7523 <https://www.rfc-editor.org/rfc/rfc7523>`_ — JWT client assertions
       (``private_key_jwt``)
     - Supported
     - ``private_key_jwt`` and ``client_secret_jwt`` token-endpoint authentication, alongside
   ...
   * - `RFC 9126 <https://www.rfc-editor.org/rfc/rfc9126>`_ — Pushed authorization
       requests (PAR)
     - Supported
     - ``par/`` endpoint (enabled by default via ``PAR_ENABLED``); advertised as
       ``pushed_authorization_request_endpoint`` in the server metadata.
...
* **FAPI 2.0** — Partial. PAR (RFC 9126) and ``private_key_jwt`` (RFC 7523) client
  authentication are now implemented, but FAPI 2.0 also mandates sender-constrained tokens
  (mTLS or DPoP), which are not, so the profile is not met.
* **MCP authorization** — Supported (configurable). DOT ships every spec the MCP
  ...
  Registration (with Client ID Metadata Documents as the MCP-preferred alternative), and,
  through the RFC 9700 gates above, an OAuth 2.1 posture. To run an
...
Gaps that block the advanced profiles, in rough priority order: sender-constrained tokens —
DPoP (RFC 9449) and mTLS (RFC 8705) — JWT access tokens (RFC 9068), and OIDC back-channel
logout.
