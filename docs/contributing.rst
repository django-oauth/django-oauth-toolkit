...
For example, to test a data migration that runs after
...
.. code-block:: python
    ...
    # Move the app to the migration state before the migration under test.
    call_command("migrate", "oauth2_provider", "0011")
    ...
    HistoricalApplication = state.apps.get_model("oauth2_provider", "Application")
    HistoricalAccessToken = state.apps.get_model("oauth2_provider", "AccessToken")
    ...
    call_command("migrate", "oauth2_provider", "0012")

When adding migrations, contributors should aim to make them safe for zero-downtime deployments whenever possible. 
In particular, avoid long-running data updates, full-table rewrites, and lock-heavy queries in migrations, 
as these can block application reads or writes during deployment and may cause downtime on large installations.

.. note:: For local debugging, you may want to inspect the SQL generated by a data 
  migration. Tools such as ``shell_plus --print-sql`` from ``django-extensions`` 
  can be useful for this. Migration tests and pull requests should not depend on them.
