...
``oauth2_provider.middleware.OIDCSessionManagementMiddleware`` to ``MIDDLEWARE`` and set
``OIDC_SESSION_MANAGEMENT_ENABLED`` to ``True`` on
``OAUTH2_PROVIDER``. You will also need to provide a string on
``OIDC_SESSION_MANAGEMENT_DEFAULT_KEY``. This setting is
...
processes::
    ...
    OAUTH2_PROVIDER = {
        ...
        "OIDC_SESSION_MANAGEMENT_DEFAULT_KEY": os.environ.get("OIDC_SESSION_MANAGEMENT_DEFAULT_KEY"),
    }
