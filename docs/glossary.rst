...
.. glossary::
    ...
    Relying Party
        ...
        :term:`OpenID Connect Provider` to authenticate resource owners — that is, the
        OIDC specialization of the OAuth2 :term:`Client` role. Django OAuth Toolkit's
        own client-side helpers live in ``oauth2_provider.client``, named for the OAuth2
        role (see :doc:`package_layout`). Note that "RP" also appears in the names of
        OpenID Provider endpoints that *serve* external relying parties (RP-Initiated
        Registration and Logout); those are provider code, not client code.
