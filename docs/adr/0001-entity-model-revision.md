...
| B | **Authentication material** | `client_secret`, `hash_client_secret`, `algorithm`, `client_type`, **`token_endpoint_auth_method`, `client_jwks`, `client_jwks_uri`** (RFC 7523, shipped) | clients that can authenticate |
| C | **Declared policy** | `redirect_uris`, `allowed_origins`, `post_logout_redirect_uris`, `authorization_grant_type`, `skip_authorization`, **`require_pushed_authorization_requests`** (RFC 9126, shipped), available scopes | all, but *source* differs |
...
Rows B and C grew materially in the 7523 and 9126 releases (§11) — the registration
concept is *accreting*, while the principal (row A) stays a single string. That
divergence is the argument in miniature.
...
## 11. Interplay with the spec roadmap — two now shipped, and they confirm the model

**Update after rebase onto current master.** Two of these have landed since this document
was first written, and both resolved the way this analysis predicted. That converts them
from speculation into in-tree evidence.

### Shipped: RFC 7523 (#1762) and RFC 9126 PAR (#1763)

**PAR references the client by a `client_id` string, not an `Application` FK.** The new
swappable model reads:
...
That is the second credential-tier entity in DOT to attribute by identifier string rather
than by FK (`DeviceGrant` was the first), decided independently of this document. **DOT's
own newest code is converging on the string-attribution pattern that Grounding B found in
five of six external servers** — while the older, FK-bound token tables stay the outlier.

**RFC 7523 made client authentication enumerable, exactly as predicted.**
`AbstractApplication` now carries `token_endpoint_auth_method` with a real choice set
(`none`, `client_secret_basic`, `client_secret_post`, `client_secret_jwt`,
`private_key_jwt`) plus `client_jwks` / `client_jwks_uri`. Invariant **I3** is therefore
no longer expressible as the `confidential`/`public` boolean — that shipped. Two
consequences for this document:

- The auth-capability question ("can this client authenticate, and how?") is now
  concretely a **property of the registration/metadata layer**, not of the principal —
  which is what §1 row B and Model 4 assert.
- A **derived (CIMD) client can now be confidential in practice**, since a metadata
  document may carry `jwks`/`jwks_uri` and `private_key_jwt` needs no pre-shared secret.
  DOT's CIMD implementation remains public-only by choice (Grounding A.2); that choice is
  now a *policy* decision sitting on top of machinery that could support more.

### A concrete drift the shipped code introduces: the identifier has three widths

The principal's identifier is now declared independently in three places, and they
disagree:

| Model | `client_id` width |
|---|---|
| `AbstractApplication` | **255** (widened for CIMD URLs) |
| `AbstractDeviceGrant` | 100 |
| `AbstractPushedAuthorizationRequest` | **100** (new) |

The CIMD work knew about this for the device flow and scoped around it — its
`GRANT_TYPE_MAP` comment notes device_code is excluded because "its grant would also need
`DeviceGrant.client_id` widened to hold a URL." **PAR then reproduced the same limitation
without the carve-out being restated.** The practical result: a CIMD client, whose
`client_id` is an https URL that may exceed 100 characters, cannot reliably use PAR — and
PAR can be made mandatory (`REQUIRE_PUSHED_AUTHORIZATION_REQUESTS`, or the per-client
`require_pushed_authorization_requests` flag), so "CIMD client + required PAR" is a
combination that can fail on a field width.

This is precisely the failure mode a first-class principal prevents: when no entity owns
"the client identifier," every new table re-declares it ad hoc and the declarations drift.
Whether or not Model 4 is adopted, **the canonical width of `client_id` should be defined
once and reused** — and the CIMD/PAR interaction needs a test either way.
...
| **RFC 8705 mTLS** | Extends the now-shipped `token_endpoint_auth_method` set with `tls_client_auth` / `self_signed_tls_client_auth`, adding cert/subject-DN material to the registration. The enumerable-methods groundwork is already in tree; 8705 is additive to it. Its cert-bound tokens share the `cnf` slot below. |
| **jwt-bearer grant (7523 §2.1)** | Distinct from the shipped §2.2 *client authentication*: an assertion **grant** would be a new `Authorization.grant_type`. The per-flow creation hooks must stay open to new grant types — today `_resolve_authorization` silently returns `None` for a grant type it does not recognise, which would leave such tokens with no lineage. |
...
Gates remaining after the two releases: (1) `Authorization` represents "what was granted"
as scope **+ `authorization_details`**, not scope alone; (2) tokens get a **`cnf`/binding
slot** shared by DPoP and mTLS, surfaced through introspection; (3) the canonical
**`client_id` width** is defined once rather than per-table. Gate (4) from the original
list — enumerable client-auth methods — **shipped in 7523** and no longer needs deciding.

---

## 12. The lifetime axis — three new considerations, and one boundary to defend

Three considerations were added to #1723 after this document was drafted
([session-scoped token lifetime](https://github.com/django-oauth/django-oauth-toolkit/issues/1723#issuecomment-5076005783),
[refresh-token lifetime unmodeled](https://github.com/django-oauth/django-oauth-toolkit/issues/1723#issuecomment-5076133925),
[no reaper for abandoned chains](https://github.com/django-oauth/django-oauth-toolkit/issues/1723#issuecomment-5076240981)).
They are one axis: **nothing durable currently represents "how long this grant lives" or
"this login is over,"** so lifetime is inferred per-token and cleanup has nothing to hang
on. Verified against current master:

- `AbstractRefreshToken` has **no lifetime field at all** — only `created`, `updated`,
  `revoked`. Expiry is implicit (`created + REFRESH_TOKEN_EXPIRE_SECONDS`) and enforced
  only by `cleartokens` deleting rows, not at validation time.
- Because each rotation mints a row with a fresh `created`, the implicit lifetime is
  inherently **sliding**; an actively-refreshed chain never reaches an absolute cap, and
  there is no way to express "this grant expires N seconds after the original
  authentication regardless of rotation."
- With stock settings an **abandoned** login's refresh token is effectively immortal:
  rotation revokes but never deletes, and the age-based reaper is opt-in.

All three want the same missing thing — **a durable anchor carrying the grant's
lifetime** — which is exactly what `Authorization` and `Session` provide. That part is
straightforwardly additive to ADR 0001, and the Cleanup section should state the
complementary direction it currently omits: it already sequences "purge
Authorizations/Sessions only *after* their token chains are gone"; the other direction —
**ending a Session (or revoking an Authorization) should be able to drive chain
reaping** — is what finally bounds the abandoned-login case.

### The boundary: `token_family` is not the session

One phrasing in those notes — "treat the refresh-token chain (`token_family`) as the
session" — should not be adopted literally, because it is the model ADR 0001 already
considered and rejected, and it collides with the ADR's own most-at-risk boundary:

- ADR 0001's alternatives list rejects **"Refresh token as the session anchor"** on four
  grounds: flows without refresh tokens still create sessions; `offline_access` refresh
  tokens must outlive the session *by definition*; one session spans many refresh tokens
  across RPs; and the `sid` must be minted at authentication time while refresh-token
  identity rotates and never appears in ID tokens.
- The ADR maps `token_family` to **`Authorization`**, not `Session`: "a rotation family
  is exactly *refresh tokens under one `Authorization`*."
- The ADR names this exact drift as the thing to police: "`Authorization` will tempt
  session-ish behavior … the logout semantics section is the line to hold in review."

**`offline_access` is the decisive counterexample.** The ADR's logout semantics say
terminating a session revokes its chains *except* `offline_access` ones. If the refresh
chain *were* the session, an `offline_access` chain would be a session that can never
end — incoherent. A refresh chain is per (user × client); a session is per (user × user
agent) and spans clients.

The underlying needs survive the correction, and they sort cleanly across the two
entities rather than collapsing into one:

| Need | Correct anchor | Why not the other |
|---|---|---|
| Orphaned ID token still resolvable for `id_token_hint` after its access token is gone | `Authorization` (durable lineage) — reachable from `Session` when there is one | Token-to-token links are what break; lineage is the fix |
| Absolute (non-sliding) grant lifetime | `Authorization` (created at the authorization event) | ROPC and client_credentials have refresh chains but **no session**, so a session anchor cannot cover them |
| "This login is over" → reap the chains | `Session` termination | Sessions span clients; an authorization does not end a login |

So: keep the `Authorization`/`Session` split, hang **grant lifetime** on `Authorization`
(covering the session-less flows) and **login lifetime** on `Session`, and let either one
drive reaping. That satisfies all three notes without reviving the rejected model.

### Note on `token_family`'s trajectory

ADR 0001 plans for `token_family` to be "retained and deprecated" once lineage moves to
the `Authorization` FK. Master has since moved the other way: #1810 added a
`revoke_family()` classmethod and a **dedicated index** on `token_family`, and #1817 made
`RefreshToken.token_checksum` unique. None of this is wrong, but the deprecation is now a
larger change than the ADR assumed, and the migration path (dual-write, or reuse
detection reading the FK while the index remains) should be stated rather than implied.
...
8. **Canonical `client_id` width (§11).** `Application` is 255 (CIMD URLs) while
   `DeviceGrant` and the new `PushedAuthorizationRequest` are 100, so a CIMD client may be
   unable to use PAR — which can be mandatory. Define the width once; add a CIMD+PAR test
   regardless of whether Model 4 lands.
9. **Where does grant lifetime live (§12)?** Absolute vs sliding, and on which entity:
   grant lifetime on `Authorization` (covers session-less ROPC/client_credentials chains)
   and login lifetime on `Session`, with either able to drive reaping — or some other
   split? Settle it before `Authorization` is frozen, since it decides whether the consent
   entity carries lifetime fields at all.
10. **`token_family` deprecation path (§12).** Master added `revoke_family()` and an index
    on `token_family` (#1810) while the ADR plans to subsume it into the `Authorization`
    FK. State the migration (dual-write? FK-based detection retaining the index?) rather
    than assuming a clean removal.
11. **Spec-readiness gates (§11).** Does the 4.0 wave ship `Authorization` with an
...
12. **Resource-server packaging (§9).** Confirm a pure introspecting resource server can
