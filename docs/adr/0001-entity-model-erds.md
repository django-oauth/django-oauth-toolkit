...
erDiagram
    ...
    APPLICATION ||..o{ PUSHED_AUTHORIZATION_REQUEST : "client_id STRING (no FK)"

    APPLICATION {
        bigint id PK
        string client_id "max_length=255 (CIMD URLs)"
        string client_secret
        string client_type
        string token_endpoint_auth_method "RFC 7523"
        string client_jwks
    ...
    DEVICE_GRANT {
        string device_code
        string client_id "STRING, not a FK - max_length=100"
    }
...
**Note the split personality (updated after RFC 9126 shipped).** The *token* tables
attribute by hard FK, but both *credential-tier* entities — `DEVICE_GRANT` and the new
`PUSHED_AUTHORIZATION_REQUEST` — attribute by `client_id` **string**. DOT's newest code is
independently converging on the string pattern that five of six external servers use
(§Grounding B), while the older token tables remain the outlier. The two dashed lines also
carry `max_length=100` against `APPLICATION.client_id`'s 255, so a CIMD URL client_id may
not fit them — see §11 of the revision document.
...
|---|---|---|---|
| **DOT current** | none | **FK** to `Application` from tokens; **string** from both credential-tier entities (device grant, PAR) | none |
