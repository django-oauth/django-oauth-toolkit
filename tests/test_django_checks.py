...
MISSING_DEFAULT_KEY = deepcopy(OIDC_SETTINGS_SESSION_MANAGEMENT)
MISSING_DEFAULT_KEY["OIDC_SESSION_MANAGEMENT_DEFAULT_KEY"] = None
...
class DjangoChecksTestCase(TestCase):
    ...
    @override_settings(OAUTH2_PROVIDER=MISSING_DEFAULT_KEY)
    def test_checks_fail_when_default_session_key_is_missing(self):
        message = "OIDC Session management is enabled, OIDC_SESSION_MANAGEMENT_DEFAULT_KEY is required."
        with self.assertRaisesMessage(SystemCheckError, message):
            call_command("check")
