...
def validate_session_management_configuration(app_configs, **kwargs):
    ...
    has_default_key = oauth2_settings.OIDC_SESSION_MANAGEMENT_DEFAULT_KEY is not None
    if oidc_session_enabled and not has_default_key:
        return [
            checks.Error(
                "OIDC Session management is enabled, OIDC_SESSION_MANAGEMENT_DEFAULT_KEY is required."
            )
        ]
    return []
