...
class SessionIFrameView(OIDCOnlyMixin, OAuthLibMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if not oauth2_settings.OIDC_SESSION_MANAGEMENT_ENABLED:
            raise Http404("OIDC session management is not enabled.")
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        for app in Application.objects.all():
            origins = set()
            for redirect_uri in app.redirect_uris.split():
                if redirect_uri:
                    parsed = urlparse(redirect_uri)
                    origin = f"{parsed.scheme}://{parsed.netloc}"
                    origins.add(origin)
            if origins:
                allowed_origins_by_client_id[app.client_id] = list(origins)

        context["allowed_origins_by_client_id"] = json.dumps(allowed_origins_by_client_id)
        return context
